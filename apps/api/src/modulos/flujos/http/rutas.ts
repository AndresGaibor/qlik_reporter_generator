...
import { crearRutasClonadoDataflow } from "./rutas-clonado-dataflow.js";
import type { DependenciasClonadoDataflow } from "./rutas-clonado-dataflow.js";
...
export function crearRutasFlujos(
  ...
  dependenciasClonado?: Omit<DependenciasClonadoDataflow, "resolverQlik">,
) {
  const rutas = new Hono();
  if (dependenciasClonado && resolverQlik) {
    rutas.route(
      "/",
      crearRutasClonadoDataflow({ ...dependenciasClonado, resolverQlik }),
    );
  }
  rutas.get("/", async (c) => {
