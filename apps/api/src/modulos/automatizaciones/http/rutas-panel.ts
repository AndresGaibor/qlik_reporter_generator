...
export function crearRutasPanelAutomatizaciones(
  ...
  rutas.post("/desde-plantilla", async (c) => {
    ...
    const tablaId = cuerpoObj.tablaId;
    const autorIngresado = typeof cuerpoObj.autor === "string" ? cuerpoObj.autor.trim() : undefined;
    const nombreFinal =
      typeof cuerpoObj.nombre === "string" && cuerpoObj.nombre.trim()
        ? cuerpoObj.nombre.trim()
        : `Reporte ${tablaId || "Personalizado"}${autorIngresado ? ` ${autorIngresado}` : ""}`;
