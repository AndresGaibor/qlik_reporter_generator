...
describe("compilarPlanABigQuery", () => {
  it("compila el fixture a CTEs BigQuery con filtro, función y LEFT JOIN", async () => {
    ...
    expect(sql).toContain(
      "`demo-proyecto.EDWH_REP.VENTAS_COMERCIAL_DIARIAS_D`",
    );
  ...
  it("traduce expresiones, DISTINCT, agregación, GROUP BY y ORDER BY", () => {
      ...
      fuentes: [
        {
          ...
          campos: ["categoria", "monto", "fecha"].map((alias) => ({
            expresion: alias,
            alias,
            dialecto: "bigquery" as const,
          })),
        },
      ],
      pasos: [
        {
          ...
          salida: "filtrada",
          condicion:
            "[monto] <> 0 AND [categoria] IN ('A', 'B') AND [fecha] IS NOT NULL",
          dialecto: "qlik",
        },
        {
          tipo: "proyectar",
          ...
          campos: [
            {
              expresion: "Upper([categoria])",
              alias: "Categoria",
              dialecto: "qlik",
            },
            {
              expresion: "If(Sum([monto]) > 100, 'ALTO', 'BAJO')",
              alias: "Nivel",
              dialecto: "qlik",
            },
        ...
        },
        {
          ...
          campos: [
            { expresion: "[Total]", direccion: "desc", dialecto: "qlik" },
          ],
        },
      ],
      salida: {
        tablaLogica: "resumen_ordenado",
        campos: ["Categoria", "Nivel", "Total", "Anio"],
      },
    ...
    expect(sql).toContain(
      "CASE WHEN SUM(`monto`) > 100 THEN 'ALTO' ELSE 'BAJO' END",
    );
  ...
  it.each(["inner", "left", "right", "full"] as const)(
    ...
    (tipo) => {
        ...
        fuentes: [
          {
            ...
            tabla: "p.d.a",
            campos: [
              { expresion: "id", alias: "id", dialecto: "bigquery" },
              { expresion: "valor", alias: "valor", dialecto: "bigquery" },
            ],
          },
          {
            id: "b",
            ...
            campos: [
              { expresion: "id", alias: "id", dialecto: "bigquery" },
              { expresion: "nombre", alias: "nombre", dialecto: "bigquery" },
            ],
          },
        ],
        pasos: [
      ...
      expect(compilarPlanABigQuery(plan).sql).toContain(
        `${tipo.toUpperCase()} JOIN`,
      );
    },
  );
  ...
  it("rechaza operaciones no soportadas antes de generar SQL", () => {
    const plan: PlanDataflow = {
      ...
      operacionesNoSoportadas: [
        { operacion: "ApplyMap", detalle: "no soportada" },
      ],
    };
