...
describe("eliminacion GCS", () => {
  test("eliminarArchivo borra exactamente el objeto solicitado", async () => {
    ...
    expect(file).toHaveBeenCalledWith(
      "POCs/TalendDescargados/byronnasimba/a.csv",
    );
  ...
  test("eliminarPrefijo borra todos los objetos contenidos y devuelve la cantidad", async () => {
    ...
    const getFiles = vi.fn().mockResolvedValue([
      [
        {
          name: "POCs/TalendDescargados/byronnasimba/carpeta/a.csv",
          delete: borrarA,
        },
        {
          name: "POCs/TalendDescargados/byronnasimba/carpeta/b.csv.gz",
          delete: borrarB,
        },
      ],
    ]);
    ...
    const total = await cliente.eliminarPrefijo(
      "POCs/TalendDescargados/byronnasimba/carpeta/",
    );
    expect(total).toBe(2);
