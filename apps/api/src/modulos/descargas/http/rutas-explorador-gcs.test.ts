...
describe("eliminacion en carpeta privada", () => {
  function appEliminar(rol: "admin" | "usuario") {
    ...
    const almacenamiento = {
      listar: async () => [],
      listarDirectorio: async () => ({ carpetas: [], archivos: [] }),
      estaFinalizada: async () => false,
      firmar: async () => "",
      eliminarArchivo: async (ruta: string) => {
        eliminados.push(ruta);
      },
      eliminarPrefijo: async (ruta: string) => {
        prefijos.push(ruta);
        return 2;
      },
    } as unknown as PuertoAlmacenamientoDescargas;
    ...
    const app = new Hono();
    app.route("/api/descargas", rutas);
    return { app, eliminados, prefijos };
  }
  it("admin elimina un archivo solo dentro de su carpeta", async () => {
    const { app, eliminados } = appEliminar("admin");
    const res = await app.request(
      "/api/descargas/carpeta/archivo?ruta=pruebagcp.csv",
      { method: "DELETE" },
    );
    expect(res.status).toBe(200);
    expect(eliminados).toEqual([
      "POCs/TalendDescargados/byronnasimba/pruebagcp.csv",
    ]);
  });
  ...
  it("admin elimina una subcarpeta completa pero no la raiz personal", async () => {
    ...
    const res = await app.request(
      "/api/descargas/carpeta/directorio?ruta=test-bq-sftp%2F",
      { method: "DELETE" },
    );
    expect(res.status).toBe(200);
    ...
    const raiz = await app.request("/api/descargas/carpeta/directorio?ruta=", {
      method: "DELETE",
    });
...
describe("seguridad de eliminacion privada", () => {
  it("rechaza traversal antes de eliminar un archivo", async () => {
    ...
    app.route("/api/descargas", rutas);
    const res = await app.request(
      "/api/descargas/carpeta/archivo?ruta=..%2Fandresgaibor%2Fsecreto.csv",
      { method: "DELETE" },
    );
    expect(res.status).toBe(422);
    expect(eliminados).toHaveLength(0);
