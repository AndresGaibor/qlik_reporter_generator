...
const CLASES: Record<
  TamMarca,
  { qlik: string; report: string; separador: string }
> = {
...
export function MarcaQlikReport({ tam = "md" }: { tam?: TamMarca }) {
  ...
  return (
    <div className="inline-flex items-center gap-2.5" aria-label="Qlik Report">
      <span
        className={`${clases.qlik} font-display font-bold leading-none tracking-[-0.045em] text-brand-600`}
      >
        Qlik
      </span>
      <span
        className={`${clases.separador} w-px bg-line-300`}
        aria-hidden="true"
      />
      <span
        className={`${clases.report} font-display font-medium leading-none tracking-tight text-ink-700`}
      >
        Report
      </span>
    </div>
