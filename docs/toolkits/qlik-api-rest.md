---
title: "REST | Qlik Developer Portal"
source_url: "https://qlik.dev/toolkits/qlik-api/rest/"
local_path: "docs/toolkits/qlik-api-rest.md"
---

Title: REST | Qlik Developer Portal


[Skip to content](https://qlik.dev/toolkits/qlik-api/rest/#start-of-content)[![Image 1: Qlik logo](https://qlik.dev/logo.svg)](https://qlik.dev/)
...
[View as Markdown](https://qlik.dev/toolkits/qlik-api/rest.md)
...
Each rest entity is exposed as its own JavaScript module. It contains all API calls and types exposed by the entity service. They can be imported from `@qlik/api` as a sub module of the package like `/spaces` in the example below. These modules both have named exports and a default export. Types are always named exports.

```
import spaces, { type Spaces } from "@qlik/api/spaces";await { data: mySpaces } = await spaces.getSpaces();
...
For more information on these entities and their APIs, see [Overview of Qlik APIs](https://qlik.dev/apis/).

## [](https://qlik.dev/toolkits/qlik-api/rest/#http-calls) HTTP Calls

HTTP calls uses the native fetch API. Each HTTP call with a response status in the 200 range will resolve to an object with structure `{ status, headers, data }`. If the status is in the >=300 range, an error will be thrown.

```
import { getSpaces } from "@qlik/api/spaces";
try {  const { status, headers, data: spaces } = await getSpaces();  // status < 300  if (spaces.data) {    // the spaces list is returned as "data"    // There are spaces  }} catch (e) {  // status >= 300  // something went wrong}
```

## [](https://qlik.dev/toolkits/qlik-api/rest/#error-handling) Error handling
...
try {  const { status } = await deleteExtension("<extension-id>");  ...} catch (e as DeleteExtensionHttpError) {    if (e.status === 404) {        // DeleteEndpoint404HttpError        e.data // < -- body of error    }}
```

## [](https://qlik.dev/toolkits/qlik-api/rest/#caching) Caching

Every GET request is cached so that subsequent calls to the same API will resolve immediately from the cache. For more information on caching, see [Features](https://qlik.dev/toolkits/qlik-api/features/).
...
*   [HTTP Calls](https://qlik.dev/toolkits/qlik-api/rest/#http-calls)
*   [Error handling](https://qlik.dev/toolkits/qlik-api/rest/#error-handling)
*   [Caching](https://qlik.dev/toolkits/qlik-api/rest/#caching)
