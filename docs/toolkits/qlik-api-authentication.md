---
title: "Authentication | Qlik Developer Portal"
source_url: "https://qlik.dev/toolkits/qlik-api/authentication/"
local_path: "docs/toolkits/qlik-api-authentication.md"
---

Title: Authentication | Qlik Developer Portal


[Skip to content](https://qlik.dev/toolkits/qlik-api/authentication/#start-of-content)[![Image 1: Qlik logo](https://qlik.dev/logo.svg)](https://qlik.dev/)
...
[View as Markdown](https://qlik.dev/toolkits/qlik-api/authentication.md)
...
## [](https://qlik.dev/toolkits/qlik-api/authentication/#the-host-config) The host config
...
const hostConfig = {  authType: "apikey",  host: "my-org.region.qlikcloud.com", // a qlikcloud tenant  apiKey: "<api-key>",};
// sets a default host config for every api requestauth.setDefaultHostConfig(hostConfig);
...
const hostConfig = {    authType: "apikey"    host: "my-org.region.qlikcloud.com", // a qlikcloud tenant    apiKey: "<api-key>",  }
const { data: mySpaces } = spaces.getSpaces({}, {  hostConfig,});
console.log(mySpaces);
```

## [](https://qlik.dev/toolkits/qlik-api/authentication/#the-auth-module) The auth module
...
const authModule = {  /** Gets the auth params needed for rest calls */  getRestCallAuthParams: () => Promise.resolve({    /** will be applied to rest requests headers */    headers: { ... },    /** will be added as query parameters to the request */    queryParams: { ... },    /** will set the credentials attribute on the rest request */    credentials: ("include"|"omit"|"same-origin"),  }),
  /** Gets params for websocket connect requests */  getWebSocketAuthParams: () => Promise.resolve({    // will be added as query parameters to the websocket connect request     queryParams: { ... }  });
  /** Will be called when there's an authentication error (e.g. user is notlogged in) during an api call. */  handleAuthenticationError: () => {    // ... maybe start a login process    return Promise.resolve();  }
  /** optional method to get auth params when web resources are fetched (such as images etc) */  getWebResourceAuthParams: () => Promise.resolve();
  /**  * Optional runtime check for validating a host configs options, return false if  * something is missing or if there are invalid properties in the host config.  */  validateHostConfig: (hostConfig) => true;}
```

## [](https://qlik.dev/toolkits/qlik-api/authentication/#the-default-auth-modules) The default auth modules

*   [Oauth2](https://qlik.dev/toolkits/qlik-api/authentication/#oauth2)
*   [Cookie](https://qlik.dev/toolkits/qlik-api/authentication/#cookie)
*   [Windows Cookie](https://qlik.dev/toolkits/qlik-api/authentication/#windows-cookie)
*   [API Key](https://qlik.dev/toolkits/qlik-api/authentication/#api-key)

### [](https://qlik.dev/toolkits/qlik-api/authentication/#oauth2) Oauth2
...
`type Oauth2AuthConfig = {  /** The URL to the cloud tenant or windows server. If scheme is excluded https is used. May include a virtual proxy prefix on windows. Any trailing slashes are stripped. */  host?: string;  /** Client ID of oauth client created by tenant administrator */  clientId: string;  /** Client ID of oauth client created by tenant administrator */  clientSecret?: string;  /** The location where the client should be redirected after obtaining the access token */  redirectUri?: string;  /** If set, store the access token in either local or session storage, otherwise not stored */  accessTokenStorage?: "session" | "local";  /** A string with comma separated values of oauth2 scopes https://oauth.net/2/scope defaults to "user_default" */  scope?: string;};`

When using OAuth2 in a browser it is recommended to set up a “redirect page” which is handy to use for pointing the redirection of an OAuth login flow when fetching the access tokens. An OAuth server will ask the client to redirect back to the hosting application after login is completed. In a single page application context this can be any URL, so the recommendation is to use this HTML template for redirections.

`<!DOCTYPE html><html lang="en">  <head>    <meta charset="utf-8" />    <script      crossorigin="anonymous"      type="application/javascript"      data-host="<tenant-url>"      src="https://cdn.jsdelivr.net/npm/@qlik/api@2/oauth-callback.iife.js"    ></script>  </head></html>`

### [](https://qlik.dev/toolkits/qlik-api/authentication/#cookie) Cookie
...
`type CookieAuthConfig = {  /** The URL to the cloud tenant or windows server. If scheme is excluded https  is used. May include a virtual proxy prefix on windows. Any trailing slashes  are stripped. */  host?: string;  /** Web Integration Id created by tenant administrator */  webIntegrationId?: string;  /** If set to false the `credentials` property will be set to same-origin  */  crossSiteCookies?: boolean;};`

### [](https://qlik.dev/toolkits/qlik-api/authentication/#windows-cookie) Windows cookie
...
`type WindowsCookieAuthConfig = {  /** The URL to the cloud tenant or windows server. If scheme is excluded https is used. May include a virtual proxy prefix on windows. Any trailing slashes are stripped. */  host?: string;  /** location of the login page, auth module will redirect to this page when an unauthenticated api call is made */  loginUri?: string;  /** If set to false the `credentials` property will be set to same-origin  */  crossSiteCookies?: boolean;};`

### [](https://qlik.dev/toolkits/qlik-api/authentication/#api-key) API key

Appends an `apiKey` to api requests that has been created by a user with the Manage API keys permission on a Qlik Cloud tenant. Typically used when running integration code in a node environment.

`type ApiKeyAuthConfig = {  /** API key created on a tenant */  apiKey: string;};`

## [](https://qlik.dev/toolkits/qlik-api/authentication/#custom-auth-module) Custom auth module
...
type CustomAuthConfig = {  configPropA: string;  optionalConfigPropB: boolean;};
// Add your CustomAuthConfig to the known global auth types
declare global {  interface QlikAuthModules {    custom: { config: CustomAuthConfig };  }}
...
const hostConfig: HostConfig = {  authType: "custom",  configPropA: "some-config-or-a-key",  optionalConfigPropB: false}
...
*   [The host config](https://qlik.dev/toolkits/qlik-api/authentication/#the-host-config)
*   [The auth module](https://qlik.dev/toolkits/qlik-api/authentication/#the-auth-module)
*   [The default auth modules](https://qlik.dev/toolkits/qlik-api/authentication/#the-default-auth-modules)
*   [Oauth2](https://qlik.dev/toolkits/qlik-api/authentication/#oauth2)
*   [Cookie](https://qlik.dev/toolkits/qlik-api/authentication/#cookie)
*   [Windows cookie](https://qlik.dev/toolkits/qlik-api/authentication/#windows-cookie)
*   [API key](https://qlik.dev/toolkits/qlik-api/authentication/#api-key)
*   [Custom auth module](https://qlik.dev/toolkits/qlik-api/authentication/#custom-auth-module)
